...
require("./neobot")(kirbotz, m, chatUpdate, store)
} catch (err) {
console.log(err)
}
})
...
Left = await getBuffer(`https://leyscoders-api.herokuapp.com/api/leave?name=${nama}&group=${encodeURIComponent(metadata.subject)}&profile=${ppuser}&apikey=IkyOgiwara`)
                ...
                if (anu.action == 'add') {
                    kirbotz.sendButWel(anu.id, cangton, `© Created By Lexxy Official `, Welcome, vtbtn)
                } else if (anu.action == 'remove') {
                   kirbotz.sendButWel(anu.id, cangtion, `© Created By Lexxy Official `, Left, bkgc)
                }
            }
        } catch (err) {
            console.log(err)
...
kirbotz.sendButWel = async (jid , text = '' , footer = '', img, but = [], options = {}) =>{
        let message = await prepareWAMessageMedia({ image: img }, { upload: kirbotz.waUploadToServer })
        var template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
        templateMessage: {
        hydratedTemplate: {
